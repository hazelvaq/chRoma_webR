...
testthat::test_that("query_collection works correctly", {
  db <- create_collection()
  db <- add_collection(db, vectors = list(c(1.2, 2.3, 4.5), c(6.7, 8.2, 9.2)), metadata = list(list(text = "This is a document", file = "source1"), list(text = "This is another document", file = "source2")))
